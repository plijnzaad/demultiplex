...
my $usage = "
Usage: $0 --barcodefile barcodes.csv --umi_len UMILENGTH --cbc_len CBCLENGTH   [--prefix name ] [--ref name ][ --allow_mm=1 ] file.bam [ file2.bam ...] 
Arguments: 
...
#check if bamfiles exist before doing anything else
for my $f (@bams) {
  die "$f: $!" unless $f; 
}
...
# everything
my ($nreads, $ninvalidUMI,  nantisense, nunisense, $nvalid, $nignored) =(0, 0, 0, 0, 0, 0); 

## mismatched cell barcodes:
my ($nmmCBC, $nmapped_mmCBC, $nrescued_mmCBC, $nmapped, $nunimapped, $nrefgenes, $nERCCs) = (0, 0, 0, 0, 0, 0, 0);
